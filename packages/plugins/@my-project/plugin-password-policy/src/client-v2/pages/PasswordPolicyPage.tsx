...
import { resetPolicyConfigCache } from '../policy-config';
...
export const PasswordPolicyPage: React.FC = () => {
  ...
  const { data: policyConfig, loading } = useRequest(async () => {
    const response = await ctx.api.resource(passwordPolicyCollectionName).get({ filterByTk: passwordPolicyRecordKey });
  ...
  const handleSubmit = useMemoizedFn(async () => {
    ...
    setSubmitting(true);
    try {
      await ctx.api.resource(passwordPolicyCollectionName).update({
