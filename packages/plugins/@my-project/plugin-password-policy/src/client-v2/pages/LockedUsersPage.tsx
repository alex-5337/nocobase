...
export const LockedUsersPage: React.FC = () => {
  ...
  const {
    ...
    const response = await ctx.api.resource(lockedUsersCollectionName).list({ paginate: false, sort: ['-updatedAt'] });
    return (response?.data?.data || []) as LockedUserRecord[];
  });

  const searchUsers = useMemoizedFn(async (keyword?: string) => {
    setFetching(true);
    try {
  ...
  const handleUnlock = useMemoizedFn(async (record: LockedUserRecord) => {
    await ctx.api.resource(lockedUsersCollectionName).destroy({ filterByTk: record.id });
    message.success(t('Unlocked successfully'));
    refresh();
  });

  const renderStatus = useMemoizedFn((_, record: LockedUserRecord) => {
